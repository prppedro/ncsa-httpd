...
int scan_cgi_header(per_request *reqInfo, int pd) 
{
   ...
   * it was:
   * if((ret = _getline(reqInfo->cgi_buf,str,HUGE_STRING_LEN-1,0,timeout)) <= 0)
   *
   ...
   */
    ...
    while(1) {
      if((ret = _getline(reqInfo->cgi_buf,str,HUGE_STRING_LEN-1,options,timeout)) <= 0)
      {
...
int cgi_stub(per_request *reqInfo, struct stat *finfo, int allow_options) 
{
  ...
  if(!pid) {
    ...
    if (reqInfo->inh_content_length > 0) {
      ...
      nBytes=_getline(reqInfo->sb, szBuf,HUGE_STRING_LEN,G_FLUSH, timeout);
      nTotalBytes = nBytes;
      if (nBytes >= 0) {
  ...
  if(!nph) {
     ...
     * _getline, but the SSI parser used buffered stdio.
     * 
     * ADC changed scan_cgi_header so that it uses G_SINGLE_CHAR when it
     * calls _getline.  Yes, this means pitiful performance for CGI scripts.
     */
...
long send_fd(per_request *reqInfo, int pd, void (*onexit)(void))
{
    ...
    if (reqInfo->cgi_buf != NULL)
      n=_getline(reqInfo->cgi_buf, buf,IOBUFSIZE,G_FLUSH,timeout);
     else 
      n = 0;
    while (1) {
	o=0;
